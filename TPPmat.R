...
pars <- function(boxes)
{
  if (missing(boxes)) boxes <- setup()
  p <- list()
  p$hivrate <- 0.001
  p$reactrate <- c(0.001,0.1); p$rapidprog <- c(0.1,0.5); p$mort <- c(0.03,0.1); p$tbmort <- c(0.2,0.4); 
      names(p$reactrate) <- names(p$rapidprog) <- names(p$mort) <- names(p$tbmort) <- boxes$Hnames  #by HIV status, made up numbers for now
  ...
  p$DSTnew <- c(0,1) #companion drugs, novel drug (doesn't depend on rif or retreatment status)
  p$eligibility <- c(1,0.9) #by HIV status
  p$targetpop <- c(1,1) #will change for MDR=(0,1), DS=(1,0), or panTB=(1,1)
  p$fail_s <- c(0.02,0.5); p$fail_s <- rep(p$fail_s, each=4) #fraction ineffective on SOC DS regimen, by rif resistance
  p$fail_r <- 0.12; p$fail_r <- rep(p$fail_r, 8) #fraction ineffective on SOC MDR regimen
  p$fail_n <- c(p$fail_s[1:3] + c(0, 0.08, 0.5), 1); p$fail_n <- rep(p$fail_n, times=2)
  p$acqres_s <- 0.005 #fraction acquiring rif resistance on SOC DS regimen
  # currently no acqres with regimen r
  p$acqres_n <- t(array(c( 0, 0.1, 0.05, 0.005,
                          0, 0, 0.4, 0.04, 
                          0, 0, 0, 0.1, 
                          0, 0, 0, 0), dim=c(4,4)))

  p$relapse_optimal <- c(0.04,0.12,0.02); names(p$relapse_optimal) <- boxes$regimens #for fully susceptible and treatment completed
  ...
  dimnames(p$relapse_resistance) <- list(boxes$Rnames, boxes$regimens)
  
  p$durations <- c(6, 18, 4); names(p$durations) <- boxes$regimens #in months, by regimen
...
makemat <- function(boxes, pars)
{
  ...
  ## Diagnosis and treatment initiation
    ...
    relapsefracs <- function(d=durations, period, lengths=Tperiod.lengths) # use relapse %s at 2, 4, and 6 months (defined in pars) for a 6 month regimen, and interpolate linearly 2--4 and 4--6 to get relapse % after any fraction of treatment course
    {
    ...
    # progression through treatment, assuming 1%/month risk of loss to follow up
