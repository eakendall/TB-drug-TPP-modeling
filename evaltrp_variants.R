...
for (i in (2:length(elementnames))) header <- append(header,  
                                                     paste0( rep(tallynames, times=2*3*3), "10",
...
if(!file.exists(paste0(location,"Exclusions","_", targetpt,DST,"_",currenttag,".csv"))) { write(header,  file=paste0(location,"Exclusions","_", targetpt,DST,"_",currenttag,".csv"), sep=",", ncol=length(header)) }
...
for (inew in rows)
{
  ...
  write(unlist(c(iter, valuevect, iresult)), file=paste0("Efficacy","_", targetpt,DST,"_",currenttag,".csv"), sep=",", append=TRUE, ncol=length(header))
}
