...
dsout <- list()
i <- 1; while(file.exists(paste0(location,"tempDSbackup/DScalibration_",tag,".",i,".csv")))
{
  dsout <- rbind(dsout, read.csv(paste0(location,"tempDSbackup/DScalibration_",tag,".",i,".csv"), header=TRUE))
  i <- i+1
}
...
for (isim in (ilimits[taskid]+1):ilimits[taskid+1]) for (isimdr in 1:Nsims_dr)
{
  ...
  # add and run to present over 25 years, with linear second line treatment scaleup to (present) max levels from years -10 to 0, and continued scaleup for 10 years beyond if rDSTall=FALSE
  ...
  write(results, append=TRUE,  sep = ",", file=paste0(location,"DRcalibration_redo_",currenttag,".csv"), ncol=length(drheader))
