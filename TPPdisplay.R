...
bpctdown <- barplot(height = 100*aperm(final_pctdown, c(2,1,3))[,,"0.5"], beside = TRUE, 
                    ...
                    main="Reduction in year 10 TB mortality with novel DS-TB regimen,\n compared to projection under current standard of care", 
...
par(mar=c(2,4,2,1), mfrow=c(1,2), oma=c(0,0,3,0))
...
mtext("Reduction in year 10 TB mortality with novel DR-TB regimen,\n compared to projection under current standard of care",side=3,outer=TRUE, cex=1.4) 
...
bres <- barplot(cbind(aperm(resource,c(1,3,2,4))["duration",2:3,,3]), beside = FALSE, 
                space=c(0.75,0.25,0.25), cex.lab=1.2, main="DR-specific treatment provided", ylim=c(0,5),
...
drltfu <- screendrout("DRcalibration_ltfu2mo.rDSTall.India_20160111.1.csv", tolerance=1.5)
outcome <- "tbdeaths"
...
header <- append(header, paste0( rtallynames, 
                                 rep(0:10, each=length(rtallynames)),
                                 "companion", 
                                 rep(c("minimal","intermediate","optimal"), each=3*11*length(rtallynames)),
                                 "barrier", 
                                 rep(c("minimal","intermediate","optimal"), each=11*length(rtallynames)) ) )

RtrajDST1 <- read.csv("Resistance_DSDSTall_rDSTall.India_20160111.idr1.csv")
RtrajDST2 <- read.csv("Resistance_DSDSTall_rDSTall.India_20160111.idr2.csv")
colnames(RtrajDST1) <- colnames(RtrajDST2) <- header
RtrajDST <- rbind(RtrajDST1,RtrajDST2)
...
Rtraj1 <- read.csv("Resistance_DSDSTnone_rDSTall.India_20160111.idr1.csv")
Rtraj2 <- read.csv("Resistance_DSDSTnone_rDSTall.India_20160111.idr2.csv")
colnames(Rtraj1) <- colnames(Rtraj2) <- header
Rtraj <- rbind(Rtraj1,Rtraj2)
drout <- rbind(alldrDST[alldrDST$idr == 1,], alldrDST[alldrDST$idr == 2,])
